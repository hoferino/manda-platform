# Create Company Overview CIM Chapter (Interactive) - v3
...
- ✏️ Adjust visual design (tell me what)
...
11. **Signal checkpoints** - "⚠️ This decision affects the rest of the narrative. Sure?"
...
- Rocket positioned at top of LTV bar, angled 45° upward-right
...
Source: company-overview.txt (Business Model, Customer Metrics, lines 180-182; Revenue Model, lines 160-161; Customer Metrics, lines 177-179)
```
...
1. **Structure Discovery Phase** - Provides logical flow reasoning upfront with narrative continuity checks
2. **Options Presentation** - Always consistent level of detail across all options
3. **Content vs Visual Separation** - Content approved first, then visual design
4. **Visual Concept Specifications** - Extreme precision: position, format, styling, icons for EVERY element
5. **Educational Moments** - Framework for explaining industry terms with examples and context
6. **User Agency** - Explicit support for non-linear workflow and section jumping
7. **Validation Loops** - Clear checkpoints with explicit approval gates
