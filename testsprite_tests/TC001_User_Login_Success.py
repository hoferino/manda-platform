...
async def run_test():
    ...
    try:
        ...
        # -> Refresh the page to verify that the user session persists and user remains logged in.
        await page.goto('http://localhost:3000/projects', timeout=10000)
        await asyncio.sleep(3)
        ...
        await expect(frame.locator('text=Welcome back, maxi.hoefer@gmx.net').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Sign Out').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Grid view').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=Table view').first).to_be_visible(timeout=30000)
        await expect(frame.locator('text=New Project').first).to_be_visible(timeout=30000)
