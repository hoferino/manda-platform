...
async def run_test():
    ...
    try:
        ...
        # -> Click on a project to open its details or knowledge explorer
        frame = context.pages[-1]
        # Click on 'Benchmark Test Deal' project to open its details or knowledge explorer
        elem = frame.locator('xpath=html/body/div[2]/main/div/div[3]/a').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Click on Knowledge Explorer menu item to open it
        frame = context.pages[-1]
        # Click Knowledge Explorer menu item
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=No contradictions detected in the knowledge explorer').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Contradictions were not detected or displayed in the knowledge explorer as required by the test plan.")
