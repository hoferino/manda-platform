...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to Q&A management page
        frame = context.pages[-1]
        # Click New Project to navigate to project creation or management options
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Fuzzy Matching Import Success').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: Q&A entries import/export with fuzzy matching did not complete successfully as per the test plan.')
